...
static NSString * iRateiOSAppStoreURLFormat = @"itms-apps://itunes.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?type=Purple+Software&id=%u";
...
//        iRateiOSAppStoreURLFormat = [NSString stringWithFormat:@"itms-apps://itunes.apple.com/%@/app/id%@",
//                            [[NSLocale preferredLanguages] objectAtIndex:0],
//                            @"ro.apps.i-tom.i-asigurare"];
...
{
    return [_messageTitle ?: [self localizedStringForKey:iRateMessageTitleKey withDefault:@"Evalueaza %@"] stringByReplacingOccurrencesOfString:@"%@" withString:self.applicationName];
}
...
{
    return _cancelButtonLabel ?: [self localizedStringForKey:iRateCancelButtonKey withDefault:@"Nu, multumesc"];
}

- (NSString *)rateButtonLabel
{
    return _rateButtonLabel ?: [self localizedStringForKey:iRateRateButtonKey withDefault:@"Evalueaza acum"];
}

- (NSString *)remindButtonLabel
{
    return _remindButtonLabel ?: [self localizedStringForKey:iRateRemindButtonKey withDefault:@"Aminteste-mi mai tarziu"];
}
...
{
        if (IS_IPHONE_5)
            myView= [[[NSBundle mainBundle] loadNibNamed:@"iRate_R4" owner:self options:nil] objectAtIndex:0];
        else
            myView= [[[NSBundle mainBundle] loadNibNamed:@"iRate" owner:self options:nil] objectAtIndex:0];
...
- (void)openRatingsPageInAppStore
{
    if (_displayAppUsingStorekitIfAvailable && [SKStoreProductViewController class])
    {
        ...
        if ([appDelegate respondsToSelector:@selector(viewController1)])
        {
            rootViewController = [appDelegate valueForKey:@"viewController1"];
        }
